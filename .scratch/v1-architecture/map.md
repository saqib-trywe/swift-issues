...
**A per-user `.pkg`**, which ticket 09 assumed and nobody had built. `make pkg`.

| Decision | Why |
| --- | --- |
| **User domain only** — `enable_localSystem="false"` | ADR 0010's install is rootless. Offering both domains would let somebody install into `/usr/local` and then wonder why an agent running as them cannot reach a database in their home |
| The agent setup lives in **`issues-server install-agent`**, not in the postinstall script | It mirrors `uninstall`, which already had to know the label and the paths. Shell in a package is the least testable code in any project; this way the plist is generated by `PropertyListSerialization` and asserted by parsing it back |
| **All three binaries ship**, only the server gets an agent | Ticket 12: the MCP executable is part of the distribution but is launched by its host, never by launchd |
| **Unsigned by default**, `PKG_SIGN_IDENTITY` to opt in | There is no Developer ID Installer certificate on this machine — only an Apple Development one, which cannot sign packages. Stated rather than silently skipped |
| `install-agent` **fails loudly if launchctl refuses** | An agent that did not load must not be reported as installed, or the operator goes looking for a server that is not running |

**Verified without installing anything**: the package was expanded with
`pkgutil --expand-full` to check its payload, domains and scripts, and then its own
`preinstall` and `postinstall` were run against a throwaway home with `launchctl`
stubbed. They wrote the right plist and called bootout before bootstrap. What this
does *not* prove is that Installer.app's user-domain path behaves the same way —
that needs a real install.

**`bootout` runs twice** on an upgrade, once from `preinstall` and once from
`install-agent`'s own idempotency. Deliberate: removing either would break a
standalone path, and both tolerate there being nothing to unload.

**A trap for the next script**: macOS ships bash 3.2, where expanding an empty
array under `set -u` is itself an error. The signing flags are written out twice
...
- **Two different Swift 6.3.3 builds are in play.** Local development uses swiftly's `swift-6.3.3-RELEASE`; CI uses Xcode's `swiftlang-6.3.3.1.3`. Same version, different binaries, and they attribute coverage regions slightly differently — which failed the no-regression gate on a sub-line difference. The tolerance now absorbs that. Worth remembering before chasing a coverage discrepancy that does not reproduce locally.
...
3. **The server has never actually run under a LaunchAgent.** The package is built
   and its scripts are exercised against a stubbed `launchctl`, but no real install
   has happened, and nothing has run for a week unattended — which is where log
   rotation, unattended restart and crash recovery would show themselves.
4. **Log rotation is still unanswered.** The agent appends to one file forever.
   `newsyslog` needs root, so a per-user install cannot use it; the honest options
   are rotating in-process or leaving it to the operator.
